...
$pageTitle = 'Update Post Comment';
$pageHeading = 'Update Post Comment'; 
$fname = 'update-post-comment.php';  
if(isset($_REQUEST['cid'])){$strid=$_REQUEST['cid'];}
  
 $sqlCoupons = "SELECT cmnt_id, post_id, cmnt_name, cmnt_email, comments, status FROM post_comments WHERE cmnt_id=".$strid;  
	$rs_coupon=$db->ExecuteQuery($sqlCoupons);            
	list($cmnt_id, $post_id, $cmnt_name, $cmnt_email, $comments, $status )=$db->FetchAsArray($rs_coupon);
...
<!--[if lt IE 9]>
  ...
  <section id="container" class="">
      ...
      <section id="main-content">
         ...
         <header class="panel-heading modal-header"> Update Comment </header>
         <div class="adv-table addcoupon text-center" > 
            <?php if ($_REQUEST['u'] == 1){?>
                        <div class="alert alert-success fade in">
                          ...
                          <strong>!</strong> Comment updated successfully.
                         </div><?php }?>          
          <div class="dataTables_wrapper">
          <form class="form" role="form" method="post" action="ajax/update-post-comments-process.php" enctype="multipart/form-data" >             
            <input  type="hidden" name="cmntid" value="<?php echo $cmnt_id;?>" >
             ...
             <label class="control-label pull-left">Post Title</label>                  
                <input  class="form-control m-bot5 typeahead" type="text" name="posttitle" id="posttitle" value="<?php echo $post_heading;?>" placeholder="Post Title*" required>  
                  </div>
                   
                    
                 <div class="form-group">
                 <label class="control-label pull-left">User Name</label>
                    <input  class="form-control m-bot5" type="text" name="cmnt_name" value="<?php echo $cmnt_name;?>" placeholder="User Name*" required>
                 </div>

               <div class="form-group">
                 <label class="control-label pull-left">User Email</label>
                    <input  class="form-control m-bot5" type="text" name="cmnt_email" value="<?php echo $cmnt_email;?>" placeholder="User Email*" required>
                 </div>
              ...
              <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">  
               ...
               <div class="form-group">
                 <label class="control-label pull-left">Comments</label>
                    <textarea rows="5" class="form-control m-bot5" id="comments" name="comments" placeholder="Comments*" required><?php echo $comments;?></textarea>
               </div>
