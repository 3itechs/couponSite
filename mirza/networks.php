          ...
          <section class="wrapper">
              ...
              <header class="panel-heading" style="padding:15px 0 45px;"> <div class="col-md-10 col-ms-10 col-xs-6"><?php echo $pageHeading;?></div>                 
                  <div class="col-md-2 col-ms-2 col-xs-6">
                  	<a href="popups/add_network_popup.php" class="btn btn-success" data-toggle="modal" data-target="#myAddModal">Add New</a>
                  </div>
                  </header>                    
                     <div class="adv-table editable-table" >
                        ...
                        <div class="alert alert-success fade in">
                          ...
                          <strong>!</strong> <?php echo $pageString;?> updated successfully.
                         </div><?php }?> 
                          ...
                          <div class="padding-top-10">
                           <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                             <div class="form-group">
                                 <label class="pull-left">Network Name</label>
                                 <input type="text" class="form-control" id="networkname" name="networkname"  placeholder="Network Name">
                              </div>
                             </div>
                            <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                           <div class="form-group">
                                    <label class="pull-left">Network Owner</label>
                                    <input type="text" class="form-control" id="networkowner" name="networkowner"  placeholder="Network Owner">                                     
                                  </div>
                              </div>
                             ...
                             <div class="panel-body col-sm-12" id="data-grid" style="height:850px;"> </div>
...
<script src="<?=DOMAINVAR?>/js/common-scripts.js"></script>
 ...
 <script type="text/javascript">	
	...
	$.ajax({
		url: url,
		type: "POST",
		data:  {rowcount:$("#rowcount").val(),dsort:$("#dsort").val(),netname:$("#networkname").val(),owner:$("#networkowner").val()},
		success: function(data){ $("#data-grid").html(data);}
	   });
	}
	
	$(document).on('click', '#btnSearch', function(){ getresult("getnetworks.php"); });
	  
	getresult("getnetworks.php");	
	
  $(document).on('click', '.btnEditAction', function(){
        var id = $(this).attr('id');
		$.ajax({
			url: "popups/update_network_popup.php?id="+id,
			type: "POST",
			data:  {},
  ...
  $(document).on('click', '.btnDeleteAction', function(){
    var id = $(this).attr('id');
	  if(confirm('Are you sure to delete it?'))
	    {
	     $.ajax({
			 url: "ajax/dell_network.php?id="+id,
			 type: "POST",
