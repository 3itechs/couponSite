          ...
          <section class="wrapper">
              ...
              <header class="panel-heading" style="padding:15px 0 45px;"> <div class="col-md-10 col-ms-10 col-xs-6"><?php echo $pageHeading;?></div>                 
                  <div class="col-md-2 col-ms-2 col-xs-6">
                  	<!--<a href="popups/add_event_popup.php" class=" btn btn-success" data-toggle="modal" data-target="#myAddModal">Add New</a>-->  <a target="_blank" href="add-event.php" class=" btn btn-success" >Add New</a>
                  </div>
                  </header>                    
                          ...
                          <div class="padding-top-10">
                           <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                             <div class="form-group">
                                 <label class="pull-left">Event Title</label>
...
<script src="<?=DOMAINVAR?>/js/common-scripts.js"></script>
 ...
 <script type="text/javascript">	
	...
	$.ajax({
		url: url,
		type: "POST",
		data:  {rowcount:$("#rowcount").val(),dsort:$("#dsort").val(),uname:$("#event_title").val(),event_img:$("#event_img").val(),sorder:$("#sortorder").val(),dsort:$("#dsort").val(),status:$("#status").val()},
		success: function(data){ $("#data-grid").html(data); $('#add-form').hide();}
	   });
	}
	
	$(document).on('click', '#btnSearch', function(){ getresult("getevents.php"); });
	  
	getresult("getevents.php");	
	
  $(document).on('click', '.btnEditAction', function(){
        var id = $(this).attr('id');
		$.ajax({
			url: "popups/update_event_popup.php?eid="+id,
			type: "POST",
			data:  {},
   ...
   $(document).on('click', '.btnImg', function(){
        var id = $(this).attr('id');
		$.ajax({
			url: "popups/img_popup.php?eid="+id,
			type: "POST",
			data:  {},
			success: function(data){ $("#myModal").html(data); }
		   });
  ...
  $(document).on('click', '.btnDeleteAction', function(){
    var id = $(this).attr('id');
	  if(confirm('Are you sure to delete it?'))
	    {
	     $.ajax({
			 url: "ajax/dell_event.php?uid="+id,
			 type: "POST",
