...
$perPage = new PerPage();
$tblName = "featured_brands";
$fileName = "get-featured-brands.php";
$country = "";
$queryCondition = "";

if(!empty($_REQUEST["title"])) { $fb_title = $_REQUEST['title'];  $queryCondition .= " and fb_title like '%".trim($fb_title)."%' "; }else{$fb_title = '';}


$orderby = " ORDER BY sid desc";
$sql = "SELECT sid, fb_title, fb_alt, fb_link, fb_img, status, added_date FROM $tblName where 1 " . $queryCondition;
	...
	<table cellpadding="10" cellspacing="1" class="table table-striped table-hover table-bordered" >
        <thead>
		 <tr>
           ...
           <th><strong>Brand Title</strong></th>
           <th><strong>Brand Link</strong></th>
        ...
        <tbody><?php
 while(list($sid, $fb_title, $fb_alt, $fb_link, $fb_img, $status, $added_date) = $db->FetchAsArray($result))
	 { ?>
    <tr id="tr-<?php echo $sid; ?>">
        <td class="sid"><?php echo $sid; ?></td>
        <td class="title"><?php echo $fb_title; ?></td>        
        <td class="link"><?php echo $fb_link; ?></td>
        <td class="link"><img height="50" width="50" src="<?php echo FEATURED_BRANDS_PATH;?><?php echo $fb_img; ?>"</td>
