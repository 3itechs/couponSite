...
$db= new DB(); 
$fname = 'main-slider.php'; 
$pageTitle = 'Manage Sliders';
$pageHeading = 'Main Slider';
$pageString = 'Sliders'; ?>
<!DOCTYPE html>
<html lang="en">
  <head>
          ...
          <section class="wrapper">
              ...
              <header class="panel-heading modal-header" style="padding:15px 0 45px;"> <div class="col-md-10 col-ms-10 col-xs-6"><?php echo $pageHeading;?></div>                 
                  <div class="col-md-2 col-ms-2 col-xs-6">
                  	<a href="popups/add_main_slider_popup.php" class=" btn btn-success" data-toggle="modal" data-target="#myAddModal">Add New</a>
                  </div>
                  </header>                    
                          ...
                          <div class="padding-top-10">
                           <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                             <div class="form-group">
                                 ...
                                 <input type="text" class="form-control" id="ms_title" name="ms_title"  placeholder="Slider Title">
                              </div>
                             </div>
...
<script src="<?=DOMAINVAR?>/js/common-scripts.js"></script>
 ...
 <script type="text/javascript">	
	...
	$.ajax({
		url: url,
		type: "POST",
		data:  {rowcount:$("#rowcount").val(),dsort:$("#dsort").val(),title:$("#ms_title").val(),sorder:$("#sortorder").val(),dsort:$("#dsort").val(),status:$("#status").val()},
		success: function(data){ $("#data-grid").html(data); $('#add-form').hide();}
	   });
	}
	
	$(document).on('click', '#btnSearch', function(){ getresult("get_main_slider.php"); });
	  
	getresult("get_main_slider.php");	
	
  $(document).on('click', '.btnEditAction', function(){
        var id = $(this).attr('id');
		$.ajax({
			url: "popups/update_main_slider_popup.php?sid="+id,
			type: "POST",
			data:  {},
  ...
  $(document).on('click', '.btnDeleteAction', function(){
    var id = $(this).attr('id');
	  if(confirm('Are you sure to delete it?'))
	    {
	     $.ajax({
			 url: "ajax/dell_main_slider.php?sid="+id,
			 type: "POST",
