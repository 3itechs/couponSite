...
<!--[if lt IE 9]>
          ...
          <section class="wrapper">
              ...
              <header class="panel-heading" style="padding:15px 0 45px;"> <div class="col-md-10 col-ms-10 col-xs-6"><?php echo $pageHeading;?></div>                 
                  <div class="col-md-2 col-ms-2 col-xs-6">
                  	<a href="popups/add_keyword_popup.php" class="btn btn-success" data-toggle="modal" data-target="#myAddModal">Add New</a>
                  </div>
                  </header>                    
                          ...
                          <div class="padding-top-10">
                           <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                             <div class="form-group">
                                 <label class="pull-left">Keyword Name</label>
                                 <input type="text" class="form-control" id="keywordname" name="keywordname"  placeholder="Keyword Name">
                            ...
                            <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                           <div class="form-group">
                                    <label class="pull-left">Parent Keyword</label>
                                     <? $sql_pc = "Select kwd_id, keyword from keywords where status = 'ENA' and 
									kwd_type = 'categories'  order by keyword  ";	?>
								  </label>                  
                             ...
                             <div class="panel-body col-sm-12" id="data-grid" style="height:550px;"> </div>
 ...
 <script type="text/javascript">	
	...
	$.ajax({
		url: url,
		type: "POST",
		data:  {rowcount:$("#rowcount").val(),dsort:$("#dsort").val(),keywordname:$("#keywordname").val(),parent:$("#parent").val(),kwd_type:$("#kwd_type").val()},
		success: function(data){ $("#data-grid").html(data);}
	   });
	}
	
	$(document).on('click', '#btnSearch', function(){ getresult("getkeywords.php"); });
	  
	getresult("getkeywords.php");	
	
  $(document).on('click', '.btnEditAction', function(){
        var id = $(this).attr('id');
		$.ajax({
			url: "popups/update_keyword_popup.php?id="+id,
			type: "POST",
			data:  {},
  ...
  $(document).on('click', '.btnDeleteAction', function(){
    var id = $(this).attr('id');
	  if(confirm('Are you sure to delete it?'))
	    {
	     $.ajax({
			 url: "ajax/dell_keyword.php?id="+id,
			 type: "POST",
