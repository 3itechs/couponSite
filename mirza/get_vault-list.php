...
$perPage = new PerPage();
$tblName = "vault_list";
$fileName = "get_vault-list.php";
$country = "";
$queryCondition = "";

if(!empty($_REQUEST["title"])) { $vl_title = $_REQUEST['title'];  $queryCondition .= " and vl_title like '%".trim($vl_title)."%' "; }else{$vl_title = '';}


$orderby = " ORDER BY sid desc";
$sql = "SELECT sid, vl_title, vl_alt, vl_link, vl_img, status, added_date FROM $tblName where 1 " . $queryCondition;
	...
	<table cellpadding="10" cellspacing="1" class="table table-striped table-hover table-bordered" >
        <thead>
		 <tr>
           ...
           <th><strong>Vault List Title</strong></th>
           <th><strong>Vault List Link</strong></th>
              <th><strong>Added Date</strong></th>
		   <th><strong>Action</strong></th>
		 </tr>
	   </thead>
        <tbody><?php
 while(list($sid, $vl_title, $vl_alt, $vl_link, $vl_img, $status, $added_date) = $db->FetchAsArray($result))
	 { ?>
    <tr id="tr-<?php echo $sid; ?>">
        <td class="sid"><?php echo $sid; ?></td>
        <td class="title"><?php echo $vl_title; ?></td>        
        <td class="link"><?php echo $vl_link; ?></td>
