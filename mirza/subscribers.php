...
$db= new DB(); 
$fname = 'subscribers.php'; 
$pageTitle = 'Manage Subscribers';
$pageHeading = 'Subscribers List';
$pageString = 'Subscribers'; ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
          ...
          <section class="wrapper">
              ...
              <header class="panel-heading modal-header" style="padding:15px 0 45px;"> <div class="col-md-10 col-ms-10 col-xs-6"><?php echo $pageHeading;?></div>                 
                  <div class="col-md-2 col-ms-2 col-xs-6">
                  	<a href="popups/add_subscriber_popup.php" class=" btn btn-success" data-toggle="modal" data-target="#myAddModal">Add New</a>
                  </div>
                  </header>                    
...
<script src="<?=DOMAINVAR?>/js/common-scripts.js"></script>
 ...
 <script type="text/javascript">	
	...
	$(document).on('click', '#btnSearch', function(){ getresult("getsubscribers.php"); });
	  
	getresult("getsubscribers.php");	
	
  $(document).on('click', '.btnEditAction', function(){
        var id = $(this).attr('id');
		$.ajax({
			url: "popups/update_subscriber_popup.php?uid="+id,
			type: "POST",
			data:  {},
  ...
  $(document).on('click', '.btnDeleteAction', function(){
    var id = $(this).attr('id');
	  if(confirm('Are you sure to delete it?'))
	    {
	     $.ajax({
			 url: "ajax/dell_subscriber.php?uid="+id,
			 type: "POST",
