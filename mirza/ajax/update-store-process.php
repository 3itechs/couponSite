...
if(isset($_POST['storename']))
    {    
		...
		$average_savings = $_POST['average_savings']; 
		...
		$store_products = addslashes(trim($_POST['store_products']));
		 if($_POST['network_id'] > 0){$network_id = $_POST['network_id']; }else{$network_id = '0' ; }
	 ...
	 $sqlUpdate = "UPDATE stores SET storename='$storename', average_savings='$average_savings', store_heading='$store_heading', store_url='$store_url', network_id='$network_id', store_products='$store_products', web_address='$web_address', primary_category = '$primary_category', child_category = '$child_category', country='$country_id', tracking_link = '$tracking_slink', imp_code = '$imp_code', store_desc='$store_desc', trademark_keywords='$trademark_keywords', more_about_desc = '$more_about_desc', edited_date= now(), network_store_name='$network_store_name', "; 		 	
$sqlUpdate.=" status='$status', top='$top', feature='$feature', meta_title='$meta_title', meta_keywords='$meta_keywords', meta_desc='$meta_desc', meta_desc='$meta_desc', store_thumbnail_alt = '$store_thumbnail_alt', ";
   ...
   if(isset($_POST['other_categories'])){
     $sql_del = "delete from store_keywords where store_id = $storeid ";	 					 
      ...
      $kwds = $_POST['other_categories'];  
		...
		$udiscount_type_ar  = $_POST['udiscount_type'];		
		$udiscount_value_ar = $_POST['udiscount_value'];
		...
		foreach( $ucouponname_ar as $key => $cn ) {  
			 ...
			 $udiscount_type = $udiscount_type_ar[$key];
			 $udiscount_value = $udiscount_value_ar[$key];
			 $ucoupon_type = $ucoupon_type_ar[$key];
...
$sql1 = "UPDATE coupons SET couponname = '$ucouponname', coupon_code = '$ucoupon_code', tracking_link = '$utracking_link', coupon_desc = '$ucoupon_desc', start_date = '$ustdate', exp_date = '$uexdate', edited_date = now(),  discount_type = '$udiscount_type', discount_value = '$udiscount_value', coupon_type = '$ucoupon_type' where coupon_id = '$ucop_id'";		
	//echo $sql1; 
...
('$couponname', '$coupon_desc', '$tracking_link', '$imp_code', '$coupon_code', 'manual', '$coupon_type','$discount_type','$discount_value', '$storeid', 'ENA', '$primary_category', now(), '$stdate', '$exdate', '0', '0', '0' , '0', '0', '$votey', '$voten')"; 
  //echo $sql; die;
