...
$db= new DB(); 
$fname = 'translation.php'; 
$pageTitle = 'Manage Translation';
$pageHeading = 'List Translation';
$pageString = 'Translation'; ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
          ...
          <section class="wrapper">
              ...
              <header class="panel-heading" style="padding:15px 0 45px;"> <div class="col-md-10 col-ms-10 col-xs-6"><?php echo $pageHeading;?></div>                 
                  <div class="col-md-2 col-ms-2 col-xs-6">
                  	<a href="add-translation.php" class="btn btn-success">Add New</a>
                  </div>
                  </header>                    
                          ...
                          <div class="padding-top-10">
                           <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                             <div class="form-group">
                                 <label class="pull-left">Page Name</label>
                                 <input type="text" class="form-control" id="ptitle" name="ptitle"  placeholder="Page Name">
                            ...
                            <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                           <div class="form-group">
                                    <label class="pull-left">Parent Page</label>
                                     <? $sql_pc = "Select category_id, category from categories where status = '1'   order by category  ";	?>
								  </label>                  
								  <select id="parent" name="parent" class="form-control m-bot5" >
									...
									while(list($kwdid, $parent_kwd) = $db->FetchAsArray($rs_sql_pc))
										{ ?>
									<option value="<?php echo $kwdid;?>"><?php echo $parent_kwd;?></option><?php }?>
								  </select>
...
<script type="text/javascript" src="<?=DOMAINVAR?>/js/jquery-1.8.3.min.js"></script>
<script type="text/javascript" src="<?=DOMAINVAR?>/js/bootstrap.min.js"></script>
...
<script type="text/javascript" src="<?=DOMAINVAR?>/js/common-scripts.js"></script>
  <!--script for this page only-->
  <script type="text/javascript" src="<?=DOMAINVAR?>/js/editable-table.js"></script>
  <!-- END JAVASCRIPTS -->
 ...
 <script type="text/javascript" src="<?=DOMAINVAR?>/js/autocomplete-jquery-ui.js"></script>  
 <script type="text/javascript">	
	function getresult(url) { 
	$.ajax({
		url: url,
		type: "POST",
		data:  {rowcount:$("#rowcount").val(),dsort:$("#dsort").val(),ptitle:$("#ptitle").val(),parent:$("#parent").val()},
		success: function(data){ $("#data-grid").html(data);}
	   });
	}
	
	$(document).on('click', '#btnSearch', function(){ getresult("gettranslation.php"); });
	  
	getresult("gettranslation.php");	
	
  $(document).on('click', '.btnEditAction', function(){
        var id = $(this).attr('id');
		$.ajax({
			url: "popups/update_translation_popup.php?id="+id,
			type: "POST",
			data:  {},
  ...
  $(document).on('click', '.btnDeleteAction', function(){
    var id = $(this).attr('id');
	  if(confirm('Are you sure to delete it?'))
	    {
	     $.ajax({
			 url: "ajax/dell_translation.php?id="+id,
			 type: "POST",
