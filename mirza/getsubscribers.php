...
$sql = "SELECT id, username, email, store_id, status, added_date FROM $tblName where 1 " . $queryCondition;
...
$perpageresult = $perPage->perpage($_GET["rowcount"], $paginationlink); ?>
	<table cellpadding="10" cellspacing="1" class="table table-striped table-hover table-bordered" >
        ...
        <tbody><?php
 while(list($id, $username, $email, $store_id, $status, $added_date) = $db->FetchAsArray($result))
	 { ?>
    <tr id="tr-<?php echo $id; ?>">
        <td class="storename"><?php echo $id; ?></td>
        <td class="storename"><?php echo $username; ?></td>        
        <td class="web_address"><?php echo $email; ?></td>
        <td class="web_address"><?php echo $db->getStoreNameFromSID($store_id);?></td>        
        <td class="added_by"><?php if($status == '1'){ echo "Enable";}else{echo "Disable";} ?></td>
