...
$db= new DB(); 
$fname = 'featured-brands.php'; 
$pageTitle = 'Manage Brands';
$pageHeading = 'Featured Brands';
$pageString = 'Brands'; ?>
<!DOCTYPE html>
<html lang="en">
  <head>
          ...
          <section class="wrapper">
              ...
              <header class="panel-heading modal-header" style="padding:15px 0 45px;"> <div class="col-md-10 col-ms-10 col-xs-6"><?php echo $pageHeading;?></div>                 
                  <div class="col-md-2 col-ms-2 col-xs-6">
                  	<a href="popups/add-featured-brands-popup.php" class=" btn btn-success" data-toggle="modal" data-target="#myAddModal">Add New</a>
                  </div>
                  </header>                    
                          ...
                          <div class="padding-top-10">
                           <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                             <div class="form-group">
                                 <label class="pull-left">Brand Title</label>
                                 <input type="text" class="form-control" id="fb_title" name="fb_title"  placeholder="Brand Title">
                              </div>
                             </div>
...
<script src="<?=DOMAINVAR?>/js/common-scripts.js"></script>
 ...
 <script type="text/javascript">	
	...
	$.ajax({
		url: url,
		type: "POST",
		data:  {rowcount:$("#rowcount").val(),dsort:$("#dsort").val(),title:$("#fb_title").val(),sorder:$("#sortorder").val(),dsort:$("#dsort").val(),status:$("#status").val()},
		success: function(data){ $("#data-grid").html(data); $('#add-form').hide();}
	   });
	}
	
	$(document).on('click', '#btnSearch', function(){ getresult("get-featured-brands.php"); });
	  
	getresult("get-featured-brands.php");	
	
  $(document).on('click', '.btnEditAction', function(){
        var id = $(this).attr('id');
		$.ajax({
			url: "popups/update-featured-brands-popup.php?sid="+id,
			type: "POST",
			data:  {},
  ...
  $(document).on('click', '.btnDeleteAction', function(){
    var id = $(this).attr('id');
	  if(confirm('Are you sure to delete it?'))
	    {
	     $.ajax({
			 url: "ajax/dell-featured-brands.php?sid="+id,
			 type: "POST",
